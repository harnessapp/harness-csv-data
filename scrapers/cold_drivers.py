...
# Sort the DataFrame by Driver and Date (to get the last 100 races per driver)
merged_df = merged_df.sort_values(by=['Driver', 'Date'], ascending=[True, False])

# Create a new DataFrame for Cold Drivers
cold_drivers = []

# Loop through each driver and calculate their stats
for driver, driver_df in merged_df.groupby('Driver'):
    # Get the last 100 races for the driver
    last_100 = driver_df.head(100)
    
    # Only include drivers with 100 or more starts
    if len(last_100) < 100:
        continue  # Skip this driver if they have fewer than 100 starts
...
# Sort by Wins (ascending), then 2nds (ascending), then 3rds (ascending)
cold_drivers_df = cold_drivers_df.sort_values(by=['Wins', '2nds', '3rds'], ascending=True)
...
print(f"Cold Drivers CSV has been created, filtered by {state_filter}!")
