...
class TrainersTablePage extends StatefulWidget {
  final String title;
  final String csvAssetPath;

  const TrainersTablePage({
    super.key,
    required this.title,
  ...
  @override
  State<TrainersTablePage> createState() => _TrainersTablePageState();
}

class _TrainersTablePageState extends State<TrainersTablePage> {
  ...
  Future<List<Map<String, dynamic>>> _buildRowsWithUpcoming(
      String trainersCsvAsset,
      String upcomingCsvAsset,
      ) async {
    // Load trainers table (structure mirrors drivers CSVs)
    final trainersRaw = await _loadCsv(trainersCsvAsset);
    if (trainersRaw.isEmpty) return [];
    final trainersBody =
    trainersRaw.length > 1 ? trainersRaw.sublist(1) : <List<dynamic>>[];

    final trainersRows = trainersBody.map<Map<String, dynamic>>((row) {
      return {
        'Trainer': row.isNotEmpty ? row[0] : '',
    ...
    // Load upcoming fields and build a lookup by trainer (expects a "Trainer" column in upcoming_fields.csv)
    List<List<dynamic>> upcomingRaw;
    try {
      upcomingRaw = await _loadCsv(upcomingCsvAsset);
    } catch (_) {
      return trainersRows.take(50).map((m) => {...m, 'Upcoming': 0, 'Next': ''}).toList();
    }
    ...
    // Note: we search for "Trainer" (not "Driver") now
    final idxTrainer = _headerIndex(header, ['Trainer']);
    ...
    final Map<String, List<Map<String, dynamic>>> byTrainer = {};
    for (final r in body) {
      final trainer =
      (idxTrainer >= 0 && idxTrainer < r.length) ? '${r[idxTrainer]}' : '';
      if (trainer.isEmpty) continue;
      byTrainer.putIfAbsent(trainer, () => []).add({
        ...
        'RaceNo': (idxRaceNo >= 0 && idxRaceNo < r.length) ? '${r[idxRaceNo]}' : '',
        'HorseNo':(idxHorseNo>= 0 && idxHorseNo< r.length) ? '${r[idxHorseNo]}' : '',
    ...
    for (final row in trainersRows) {
      final name = '${row['Trainer']}'.trim();
      final list = byTrainer[name] ?? [];
              ...
              child: Column(
                children: [
                  ...
                  Container(
                    ...
                    child: const Row(
                      children: [
                        Expanded(flex: 3, child: Text('Trainer', style: TextStyle(fontWeight: FontWeight.bold))),
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Container(
                          ...
                          child: Row(
                            children: [
                              // Trainer
                              Expanded(flex: 3, child: Text('${d['Trainer'] ?? 'Unknown'}')),
